...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        dW_word_embed = word_embedding_backward(dx, word_embed_cache)
        
        grads = {
          'W_vocab':dW_vocab, 
          'Wx':dWx, 
          'Wh':dWh,
          'W_embed':dW_word_embed, 
          'W_proj':dW_proj, 
          'b_vocab':db_vocab,           
          'b':db,           
          'b_proj':db_proj
    ...
    def sample(self, features, max_length=30):
        ...
        x = W_embed[self._start] * np.ones((N, W)) 

        for i in range(0, max_length):
            if self.cell_type == 'lstm':
              h0, c0, cache = lstm_step_forward(x, h0, c0, Wx, Wh, b)
            else:
              h0, cache = rnn_step_forward(x, h0, Wx, Wh, b)
